...
def animate(i, start_time, ax, scatters, labels, paths, time_step_days=1):
    """
    ...
    :param paths: List of line objects for planet paths
    ...
    # Update the positions of the planets and their paths
    for idx, planet in enumerate(PLANETS):
        ...
        labels[idx].set_position((x, y))
        
        # Update path (append new position to the path)
        paths[idx].set_xdata(np.append(paths[idx].get_xdata(), x))
        paths[idx].set_ydata(np.append(paths[idx].get_ydata(), y))
...
def main():
    ...
    # Initialize scatter plots, labels, and paths for planets
    scatters = []
    labels = []
    paths = []
    ...
    for planet in PLANETS:
        ...
        # Path (line) for the planet
        path, = ax.plot([x], [y], linestyle='-', linewidth=1, alpha=0.7)
        paths.append(path)
    ...
    ani = FuncAnimation(
        ...
        fargs=(start_time, ax, scatters, labels, paths, time_step_days),
        repeat=False
    )
