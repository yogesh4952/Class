...
def create_live_plot(start_year, start_month, start_day, num_days=365, interval_days=1):
    ...
    fig.update_layout(
        ...
        sliders=[{
            "active": 5,  # Default zoom level
            "steps": [
                {"method": "relayout", "label": f"{i}x", "args": [{"xaxis.range": [-i * 10, i * 10], "yaxis.range": [-i * 10, i * 10]}]}
                for i in range(1, 11)  # Zoom levels from 1x to 10x
            ],
            "pad": {"t": 50},
            "len": 0.9,
            "x": 0.1,
    ...
    frames = []
    for i, (time, positions) in enumerate(zip(times, all_positions)):
        frame_data = []
        ...
        # Update the date annotation for this frame
        date_text = f"Date: {time.iso[:10]}"
        frames.append(go.Frame(data=frame_data, layout={"annotations": [{
            "xref": "paper", "yref": "paper",
            "x": 0.95, "y": 0.95,
            "text": date_text,
            "showarrow": False,
